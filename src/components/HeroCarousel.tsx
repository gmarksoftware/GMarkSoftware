...
const GramUnnatiLogo = () => (
  <div className="bg-white p-2 rounded-lg flex flex-col items-center justify-center shadow-lg w-[80px] h-[80px] mx-auto select-none">
    <svg className="w-8 h-8" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
      {/* Left Hook (Dark Green) */}
      <path 
        d="M 95,45 C 70,25 40,45 40,85 C 40,125 70,145 100,145 C 125,145 140,125 140,105 L 112,105" 
        stroke="#0F4626" 
        strokeWidth="24" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
      />
      {/* Right Hook (Lime Green) */}
      <path 
        d="M 105,45 C 130,25 160,45 160,85 C 160,125 130,145 100,145 C 75,145 60,125 60,105 L 88,105" 
        stroke="#9CCC3C" 
        strokeWidth="24" 
        strokeLinecap="round" 
        strokeLinejoin="round" 
    ...
    <span className="text-[10px] font-extrabold text-[#111827] tracking-tight leading-none mt-1">GramUnnati</span>
    <span className="text-[3.2px] text-[#6b7280] text-center leading-none mt-1 tracking-wide">Connecting Farmers to the Future</span>
  </div>
);
...
export default function HeroCarousel({ onCTA }: { onCTA: () => void }) {
  ...
  const slides: Slide[] = [
    {
      ...
      btnClass: 'bg-accent-red text-[#410005] hover:bg-accent-red-hover shadow-accent-red/25 hover:shadow-accent-red/40 hover:-translate-y-0.5',
    },
    {
      id: 2,
      type: 'agriculture',
      ...
      btnClass: 'bg-accent-red text-[#410005] hover:bg-accent-red-hover shadow-accent-red/25 hover:shadow-accent-red/40 hover:-translate-y-0.5',
      stats: [
        { label: 'Farmers', value: '500k+' },
        { label: 'Acres', value: '1.2M' },
  ...
  return (
    <section 
      ...
      className="relative py-20 lg:py-28 overflow-hidden bg-transparent w-full border-t border-border-color"
        ...
        <div className="relative w-full min-h-[500px] lg:min-h-[420px]">
          {slides.map((slide, index) => {
            ...
            return (
              <div
                key={slide.id}
                className={`grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center w-full transition-all duration-700 ease-in-out ${
                  isActive
                    ...
                    <div className="relative w-[130px] h-[250px] rounded-[28px] border-[5px] border-text-secondary/40 bg-[#05070D] flex flex-col justify-between p-3 shadow-2xl overflow-hidden">
                      ...
                      {slide.type === 'maintenance' ? (
                        <div className="flex-1 flex flex-col justify-center items-center py-2">
                          <div className="bg-white p-1 rounded-lg flex items-center justify-center shadow-lg w-[60px] h-[60px] select-none mx-auto mb-1">
                            <svg className="w-full h-full" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <defs>
                          ...
                          <div className="w-full px-1 flex flex-col gap-1 mt-1">
                            <span className="text-[6.5px] font-mono font-bold text-text-secondary uppercase tracking-widest text-center leading-none mb-0.5">Change Management</span>
                            <div className="flex flex-col gap-1 w-full">
                              {/* Planned Change */}
                              <div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 px-1.5 py-0.5 rounded-sm">
                                <span className="text-[5.5px] font-bold text-emerald-400 font-sans tracking-wide">Planned</span>
                                <span className="text-[5px] font-mono text-emerald-400/80 bg-emerald-500/10 px-1 rounded-sm">1</span>
                              </div>
                              {/* Unplanned Change */}
                              <div className="flex items-center justify-between bg-accent-red/10 border border-accent-red/20 px-1.5 py-0.5 rounded-sm">
                                <span className="text-[5.5px] font-bold text-accent-red font-sans tracking-wide">Unplanned</span>
                                <span className="text-[5px] font-mono text-accent-red/80 bg-accent-red/10 px-1 rounded-sm">2</span>
                              </div>
                              {/* Abnormal Change */}
                              <div className="flex items-center justify-between bg-amber-500/10 border border-amber-500/20 px-1.5 py-0.5 rounded-sm">
                                <span className="text-[5.5px] font-bold text-amber-400 font-sans tracking-wide">Abnormal</span>
                                <span className="text-[5px] font-mono text-amber-400/80 bg-amber-500/10 px-1 rounded-sm">3</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col justify-center items-center">
                          ...
                          <div className="flex gap-2.5 mt-3">
                            <div className="w-7 h-7 rounded bg-accent-red/15 border border-accent-red/30 flex items-center justify-center text-accent-red">
                              <span className="material-symbols-outlined text-xs">rss_feed</span>
                            </div>
                            <div className="w-7 h-7 rounded bg-white/5 border border-white/10 flex items-center justify-center text-text-secondary">
                ...
                <div className="lg:col-span-5 flex flex-col gap-6 select-text text-left">
                  ...
                  <div className="flex flex-wrap gap-4 pt-2">
                    ...
                    {slide.secondaryBtnText && (
                      <button 
                        onClick={onCTA}
                        className="border border-accent-red/30 text-accent-red hover:bg-accent-red/10 px-8 py-3.5 font-bold text-sm uppercase tracking-wider rounded transition-all duration-300 cursor-pointer"
                      >
                        {slide.secondaryBtnText}
                      </button>
                    )}
